...
import './salesprogresspivot.css';

const SalesProgressPivot = ({ units }) => {
  ...
  const getYearMonth = (unit) => {
    if (!unit.reservation_date) return null;
  ...
  // Set first year as active by default (moved out of useMemo — side effects don't belong there)
  useEffect(() => {
    if (progressData.years.length > 0 && activeYear === null && !comparisonMode) {
      setActiveYear(progressData.years[0]);
    }
  }, [progressData.years, activeYear, comparisonMode]);

  // Stable helper — depends only on progressData
  const getAvailableMonths = useCallback((year) => {
    if (!year) return [];
    const monthsSet = new Set();
    progressData.cityData.forEach(city => {
  ...
  const displayMonths = yearsAnalysisMode 
    ? progressData.years.map(year => ({ year, month: null, isYearTotal: true }))
    : comparisonMode 
      ? comparisonPeriods 
  ...
  const resetToDefault = () => {
    ...
    if (progressData.years.length > 0) {
      setActiveYear(progressData.years[0]);
    }
  };
  ...
  const toggleCity = (cityName) => setExpandedCities(prev => ({ 
    ...prev, 
  ...
  return (
    <div className="sales-progress-container">
      ...
      <div className="sales-progress-controls">
      ...
      <div className="sales-progress-scroll">
        <table className="sales-progress-table">
          <thead>
          ...
          <tbody>
            {visibleRows.cities && progressData.cityData.map(city => {
...
export default SalesProgressPivot;